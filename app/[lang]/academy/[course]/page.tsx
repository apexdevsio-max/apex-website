...
export default async function CoursePage({ params }: Props) {
  ...
  const duration = mock?.duration[lang] ?? "";

  const lessons = mdxCourse?.lessons.length
    ? mdxCourse.lessons.map((lesson) => ({
        slug: lesson.slug,
        title: lesson.title,
        summary: lesson.summary,
      }))
    : (mock?.lessons ?? []).map((lesson) => ({
        slug: lesson.slug,
        title: lesson[lang],
        summary: "",
      }));
    ...
    >
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          {isAr ? "العودة إلى الأكاديمية" : "Back to Academy"}
        </Link>
        ...
        <div
          className={`flex items-center gap-4 mb-4 text-sm flex-wrap ${isAr ? "flex-row-reverse" : ""}`}
          style={{ color: "var(--color-secondary-text)" }}
        >
          <span>
          ...
          {lessons.map((lesson, index) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p
                  className={`font-semibold text-sm truncate ${isAr ? "font-ar" : "font-en"}`}
                ...
                {lesson.summary && (
                  <p
                    className={`text-xs mt-0.5 truncate ${isAr ? "font-ar" : "font-en"}`}
                    style={{ color: "var(--color-secondary-text)" }}
                  >
                    {lesson.summary}
                  </p>
        ...
        <div className={`flex flex-wrap gap-4 ${isAr ? "flex-row-reverse" : ""}`}>
          {lessons[0] && (
            <Link
              href={`/${lang}/academy/${courseSlug}/${lessons[0].slug}`}
