...
function StatCounter({ value, label }: { value: string; label: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-1.5 text-sm" style={{ color: "var(--color-secondary-text)" }}>
        {label}
      </div>
    </div>
  );
...
export function AboutSection({
  ...
  return (
    <section
      ...
      style={{
        background:
          "linear-gradient(180deg, var(--color-background) 0%, color-mix(in srgb, var(--color-primary) 4%, var(--color-background)) 100%)",
      }}
    >
      <div
      ...
      <div className="max-w-6xl mx-auto">
        <Reveal className="text-center mb-16" threshold={0.12} offsetY={24} durationMs={700}>
        ...
        <div
          className={`grid md:grid-cols-2 gap-12 md:gap-16 items-center ${isAr ? "direction-rtl" : ""}`}
          dir={isAr ? "rtl" : "ltr"}
        >
          <Reveal delay={150} threshold={0.12} offsetY={24} durationMs={700}>
            <p
              className={`leading-loose mb-8 ${isAr ? "font-ar" : "font-en"}`}
              style={{
            ...
            <div className="flex flex-col gap-4">
              {about.checks.map((item: string, i: number) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  />
                  <span
                    className={`font-medium ${isAr ? "font-ar" : "font-en"}`}
                    style={{
          ...
          <Reveal delay={250} threshold={0.12} offsetY={24} durationMs={700}>
            <div
              className="relative rounded-3xl p-10 border overflow-hidden apex-card-hover"
              style={{
                background:
                  "linear-gradient(135deg, color-mix(in srgb, var(--color-primary) 8%, transparent), color-mix(in srgb, var(--color-gold) 6%, transparent))",
                borderColor: "color-mix(in srgb, var(--color-primary) 25%, transparent)",
              }}
            >
              <div
                className="absolute rounded-full border pointer-events-none -inset-e-6 -top-6"
                style={{
                  width: "120px",
                  height: "120px",
              ...
              <p
                className={`leading-relaxed mb-6 ${isAr ? "font-ar" : "font-en"}`}
                style={{
                  fontSize: "clamp(13px, 1.4vw, 15px)",
                  color: "var(--color-secondary-text)",
                }}
