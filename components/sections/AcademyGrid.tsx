...
function CourseCard({ course, lang }: {
  course: typeof MOCK_COURSES[0]; lang: Locale;
}) {
  const isAr    = lang === "ar";
    ...
    >
      <div className="relative flex items-center justify-center"
        ...
        <div className="absolute rounded-full pointer-events-none"
          style={{ width:"150px",height:"150px",top:"50%",left:"50%",transform:"translate(-50%,-50%)",
            background:`radial-gradient(circle,${course.accentColor}30 0%,transparent 70%)` }}
          aria-hidden="true" />
        <span style={{ fontSize:"56px",filter:"drop-shadow(0 0 18px rgba(255,255,255,0.28))",
          transition:"transform 0.3s", transform:hov?"scale(1.12)":"scale(1)" }}>
          {course.emoji}
        </span>
        <div className="absolute top-3 px-2.5 py-0.5 rounded-full text-[10px] font-bold"
          style={{ [isAr?"left":"right"]:"10px",
            background:`color-mix(in srgb,${levelColor} 18%,rgba(0,0,0,0.55))`,
            border:`1px solid ${levelColor}55`, color:levelColor }}>
          {content.level}
        </div>
      </div>

      <div className="flex flex-col flex-1 p-6" dir={isAr?"rtl":"ltr"}>
        <div className={`flex items-center gap-3 mb-3 text-xs flex-wrap ${isAr?"flex-row-reverse":""}`}
          style={{ color:"var(--color-secondary-text)" }}>
          <span>📚 {course.lessonsCount} {isAr?"درس":"lessons"}</span>
          <span>·</span>
        ...
        <p className={`text-sm leading-relaxed flex-1 mb-4 ${isAr?"font-ar":"font-en"}`}
          style={{ color:"var(--color-secondary-text)" }}>
          {content.summary}
        </p>
        ...
        <div className="mb-4 space-y-1.5">
          {course.lessons.slice(0, 2).map((l, i) => (
            <div key={l.slug} className={`flex items-center gap-2 text-xs ${isAr?"flex-row-reverse":""}`}
              style={{ color:"var(--color-secondary-text)" }}>
              <span className="w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold shrink-0"
                style={{ background:`color-mix(in srgb,${course.accentColor} 18%,transparent)`, color:course.accentColor }}>
                {i + 1}
              </span>
              <span className={isAr?"font-ar":"font-en"}>{l[lang]}</span>
            </div>
          ))}
          {course.lessons.length > 2 && (
            <div className={`text-xs ${isAr?"font-ar text-right":"font-en"}`}
...
export function AcademyGrid({
  lang, dictionary, mdxCourses,
}: {
  lang: Locale; dictionary: Dictionary; mdxCourses: AcademyCourse[];
}) {
  const isAr = lang === "ar";
  const cats = isAr ? CATS_AR : CATS_EN;
  const [activeFilter, setActiveFilter] = useState("all");

  const courses = mdxCourses.length > 0
    ? mdxCourses.map((c, i) => ({
        ...MOCK_COURSES[i % MOCK_COURSES.length],
        slug: c.slug,
        [lang]: { ...MOCK_COURSES[i % MOCK_COURSES.length][lang], title: c.title, summary: c.summary },
      }))
    : MOCK_COURSES;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <Reveal className="text-center mb-14">
          ...
          <p className={`mt-4 mx-auto leading-relaxed ${isAr?"font-ar":"font-en"}`}
            ...
            {isAr
              ? "دورات تقنية عملية مصممة لتحوّلك من مبتدئ إلى محترف"
              : "Practical technical courses designed to take you from beginner to professional"}
          </p>
        </Reveal>

        <Reveal delay={60}>
          <div className="grid grid-cols-3 gap-4 mb-12 rounded-2xl p-6 border"
            ...
            {[
              ...
              <div key={i} className="text-center">
                <div className="font-bold" style={{ fontSize:"clamp(22px,3vw,34px)", color:"var(--color-primary)", fontFamily:"var(--font-en)" }}>
                  {s.value}
                </div>
                <div className={`mt-1 text-xs ${isAr?"font-ar":"font-en"}`} style={{ color:"var(--color-secondary-text)" }}>
                  {s.label}
                </div>
              </div>
            ))}
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20" style={{ color:"var(--color-secondary-text)" }}>
            ...
            <p className={isAr?"font-ar":"font-en"}>
              {isAr?"لا توجد دورات في هذا المستوى حالياً":"No courses at this level yet"}
            </p>
          </div>
        )}

        <Reveal delay={100}>
          <div className="mt-16 rounded-3xl p-10 text-center border relative overflow-hidden"
            style={{ background:"color-mix(in srgb,var(--color-primary) 5%,var(--color-card))",
            ...
            <div className="relative z-1">
              ...
              <p className={`mb-6 text-sm ${isAr?"font-ar":"font-en"}`} style={{ color:"var(--color-secondary-text)" }}>
                {isAr
