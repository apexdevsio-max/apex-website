...
const TikTokIcon = () => (
  <svg width="17" height="17" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    <path d="M16 1a5 5 0 005 5v3a8 8 0 01-5-1.7V15a7 7 0 11-7-7h1v3h-1a4 4 0 104 4V1h3z" />
  </svg>
);
...
export function Footer({ lang, dictionary }: Props) {
  const isAr   = lang === "ar";
  ...
  const getSocialHref = (key: string) =>
    (socialData as Record<string, string>)[key] ?? "#";
    ...
    >
      <div className="max-w-7xl mx-auto px-6 pt-16 pb-8" dir={isAr ? "rtl" : "ltr"}>
        ...
        <div className="grid gap-12 md:grid-cols-[2fr_1fr_1fr] mb-14">
              ...
              {socials.map(({ key, icon, label }) => (
                <a
                  ...
                  onMouseEnter={(e) => {
                    ...
                    el.style.borderColor = "var(--color-footer-primary)";
                    el.style.background  = "color-mix(in srgb, var(--color-footer-primary) 14%, transparent)";
                    el.style.color       = "var(--color-footer-primary)";
                  }}
                  onMouseLeave={(e) => {
                    const el = e.currentTarget as HTMLAnchorElement;
                    el.style.borderColor = "var(--color-footer-border)";
                    el.style.background  = "transparent";
                    el.style.color       = "var(--color-footer-text)";
                  }}
                >
                  {icon}
                </a>
          ...
          <div>
            ...
            <ul className="space-y-1">
              {footer.links.map((link: { label: string; path: string }) => (
                <li key={link.path}>
                  <Link
                    ...
                    onMouseLeave={(e) =>
                      ((e.currentTarget as HTMLAnchorElement).style.color = "var(--color-footer-text)")
                    }
                  >
                    {link.label}
                  </Link>
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(90deg, transparent, var(--color-footer-primary), var(--color-gold), var(--color-footer-primary), transparent)",
            opacity: 0.4,
          }}
          aria-hidden="true"
        />
          ...
          <div className="flex gap-5">
            {[
              ...
              <Link
                ...
                onMouseLeave={(e) => {
                  (e.currentTarget as HTMLAnchorElement).style.color = "var(--color-footer-text-muted)";
                }}
              >
                {label}
              </Link>
